...
func TestCompareHTML(t *testing.T) {
	t.Run("identical simple HTML matches", func(t *testing.T) {
		...
		tb := &testing.T{}
		result := helpers.CompareHTML(tb, html, html, nil, nil)
		require.True(t, result)
	})

	t.Run("different tag names don't match", func(t *testing.T) {
		tb := &testing.T{}
		result := helpers.CompareHTML(tb, []byte("<div></div>"), []byte("<span></span>"), nil, nil)
		require.False(t, result)
	})

	t.Run("different text content doesn't match", func(t *testing.T) {
		tb := &testing.T{}
		result := helpers.CompareHTML(tb, []byte("<p>hello</p>"), []byte("<p>world</p>"), nil, nil)
		require.False(t, result)
	})

	t.Run("whitespace in text nodes is ignored", func(t *testing.T) {
		tb := &testing.T{}
		result := helpers.CompareHTML(tb, []byte("<p>hello</p>"), []byte("<p>  hello  </p>"), nil, nil)
		require.True(t, result)
	})

	t.Run("attribute order doesn't matter", func(t *testing.T) {
		tb := &testing.T{}
		result := helpers.CompareHTML(
			tb,
	...
	t.Run("different attribute values don't match", func(t *testing.T) {
		tb := &testing.T{}
		result := helpers.CompareHTML(tb, []byte(`<div class="a"></div>`), []byte(`<div class="b"></div>`), nil, nil)
		require.False(t, result)
	})
	...
	t.Run("different child count doesn't match", func(t *testing.T) {
		tb := &testing.T{}
		result := helpers.CompareHTML(tb, []byte("<div><p></p><p></p></div>"), []byte("<div><p></p></div>"), nil, nil)
		require.False(t, result)
	})

	t.Run("empty text nodes are ignored", func(t *testing.T) {
		tb := &testing.T{}
		result := helpers.CompareHTML(tb, []byte("<div></div>"), []byte("<div>   </div>"), nil, nil)
		require.True(t, result)
	})
	...
	t.Run("mixed attributes and nesting", func(t *testing.T) {
		...
		result := helpers.CompareHTML(tb, html1, html2, nil, nil)
		require.True(t, result)
	})
